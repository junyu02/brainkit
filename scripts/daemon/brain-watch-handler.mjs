...
function appendErrorLog(stage, message) {
  try {
    assertInsideVault(ERRORS_MD_PATH, VAULT_ROOT);
...
function atomicWriteFileSync(filePath, content, encoding) {
  ...
  assertInsideVault(filePath, VAULT_ROOT);
...
async function processProject(mapping) {
  ...
  try {
    if (!/^01-项目\/(?!\.{1,2}$)[^/\\]+$/.test(mapping.vaultDir)) throw new Error('Invalid project vault directory');
    assertInsideVault(join(VAULT_ROOT, mapping.vaultDir, '_index.md'), VAULT_ROOT);
    // Get dir mtime
