...
function restoreBackupEntry(backupDir, vaultRoot, entry, backupFiles) {
...
function restoreManifest(backupDir, vaultRoot, manifest, backupFiles) {
  const path = join(vaultRoot, MANIFEST_RELATIVE_PATH);
  const current = hashFileOrNull(path);
  if (current !== manifest.beforeHash && current !== manifest.afterHash) {
...
function rollbackTransaction(backupDir, vaultRoot, recovery) {
  ...
  // Validate the entire restore set before changing the first file. The
  // per-file checks above still catch drift after this preflight.
  const vaultReal = realpathSync(vaultRoot);
  if (vaultReal !== recovery.vaultRoot) throw new PublishError('rollback vault identity changed', { code: 'target-drift' });
  for (const entry of [...recovery.entries, { ...recovery.manifest, target: MANIFEST_RELATIVE_PATH }]) {
    const { lexical: targetPath } = validateMappedPath(vaultRoot, vaultReal, entry.target, { allowMissing: true, label: 'rollback target' });
    const current = hashFileOrNull(targetPath);
    if (current !== entry.beforeHash && current !== entry.afterHash) {
      throw new PublishError(`recovery refuses third value for ${entry.target}`, { code: 'third-value' });
    }
...
function publish(repoRoot, vaultRoot, whitelist, manifest, launchctl) {
  ...
  try {
    ...
    if (frozen && !rollbackError) {
      try { restoreServices(services, launchctl); }
      catch (failure) { rollbackError ||= failure; }
    }
    const detail = rollbackError ? `${error.message}; rollback failed: ${rollbackError.message}; service state was not restored` : error.message;
    throw new PublishError(detail, { code: rollbackError ? 'rollback-failed' : error.code || 'transaction-failed', backupDir });
  }
}
...
function recover(repoRoot, vaultRoot, whitelist, backupDirInput, launchctl) {
  ...
  try {
    ...
    for (const entry of recovery.entries) {
      restoreBackupEntry(backupDir, vaultRoot, entry, backupFiles);
    }
    restoreManifest(backupDir, vaultRoot, recovery.manifest, backupFiles);
