...
import {
  ...
  mkdirSync, linkSync, unlinkSync, lstatSync
} from 'node:fs';
...
function archiveCollisions(candidates) {
  ...
  for (const name of DOMAIN_INDEX_FILES) {
    try { assertMetadataPath(join(MEMORY_DIR, name)); }
    catch (error) { problems.push(error.message); }
  }
  const claimed = new Map();
  for (const c of candidates) {
    try {
      assertInsideVault(c.abs_path, VAULT_ROOT);
      assertInsideVault(c.archive_path, VAULT_ROOT);
      assertInsideVault(INTENT_MAP_PATH, VAULT_ROOT);
    } catch (error) { problems.push(error.message); }
...
function moveNoClobber(from, to) {
  assertInsideVault(from, VAULT_ROOT);
  assertInsideVault(to, VAULT_ROOT);
  linkSync(from, to);
  unlinkSync(from);
}
...
function snapshotFiles(paths) {
  return paths.map(path => { assertMetadataPath(path); return path; })
    .filter(p => existsSync(p)).map(p => [p, readFileSync(p)]);
}
...
function assertMetadataPath(path) {
  if (path === INTENT_MAP_PATH) assertInsideVault(path, VAULT_ROOT);
  else {
    if (!DOMAIN_INDEX_FILES.some(name => path === join(MEMORY_DIR, name))) throw new Error('Unknown memory index');
    assertInsideVault(path, MEMORY_DIR);
  }
  try {
    const stat = lstatSync(path);
    if (!stat.isFile() || stat.nlink !== 1) throw new Error(`Index must be a single-link regular file: ${path}`);
...
function appendLineToArchiveIndex(title, newAbsPath, description) {
  ...
  assertMetadataPath(archiveIndexPath);
...
function updateIntentMapPaths(oldAbsPath, newAbsPath) {
  assertMetadataPath(INTENT_MAP_PATH);
