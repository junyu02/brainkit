...
import {
  ...
  readdirSync, realpathSync, renameSync, statSync, lstatSync, unlinkSync, writeFileSync, writeSync,
} from 'node:fs';
...
import { isMain, loadObserveEnv } from '../lib/plist-render.mjs';
...
const PROGRESS_PATH = join(CACHE, 'harvest-progress.json');
...
function usage() {
  return 'Usage: node brain-harvest.mjs cluster [--limit N] [--since YYYY-MM-DD] [--dry-run] [--force-regen] [--continue-scan]\n'
...
function parseArgs(argv) {
  ...
  const args = { command, limit: Infinity, since: null, dryRun: false, forceRegen: false, continueScan: false, port: 8843 };
  for (let i = 1; i < argv.length; i += 1) {
    const arg = argv[i];
    if (arg === '--dry-run' && command === 'cluster') args.dryRun = true;
    else if (arg === '--force-regen' && command === 'cluster') args.forceRegen = true;
    else if (arg === '--continue-scan' && command === 'cluster') args.continueScan = true;
...
function assertStatePath(path) {
  assertInsideVault(path, VAULT_ROOT);
  try {
    const info = lstatSync(path);
    if (!info.isFile() || info.nlink !== 1) throw new Error(`harvest state must be a single-link regular file: ${path}`);
...
function acquireLock() {
  assertInsideVault(CACHE, VAULT_ROOT);
...
function scanObservations({ since, consumed, locked, limit, after = null, countRemaining = false }) {
  const items = [];
  const errors = [];
  let remaining = 0;
  for (const path of walkMarkdown(OBSERVATIONS).sort()) {
    try {
      ...
      if (rel.startsWith('08-观察/chronicle-')) continue;
      const item = parseObservationMarkdown(readFileSync(path, 'utf8'), rel);
      if (!observationEligible(item, { since, consumed, locked })) continue;
      if (items.length < limit) items.push(item);
      else if (countRemaining) remaining += 1;
      if (!countRemaining && items.length >= limit) break;
    } catch (error) {
...
export function judgeAutoCandidate(candidate, judge) {
  if (candidate.evidence?.some(item => /^08-观察\/chronicle-/.test(item.path))) {
    return { decision: 'skip', reason: '被动屏幕观察未经源核实，保留观察层' };
  }
...
export async function callLlm(env, messages, model = env.OBSERVE_MODEL, {
  timeoutMs = 120_000, retryDelays = LLM_NETWORK_RETRY_DELAYS_MS, fetchImpl = fetch,
} = {}) {
  return (await requestJson(env, messages, { model, maxTokens: 16_384, timeoutMs, retryDelays, fetchImpl })).data;
}
...
export async function firstLayer(env, batches, options, checkpoint, { completed = {}, onBatch = () => {}, request = schemaRetry } = {}) {
  const candidates = [];
  const errors = [];
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    try {
      const batch = batches[batchIndex];
      const messages = firstLayerMessages(batch, options, checkpoint);
      const key = createHash('sha256').update(JSON.stringify({ model: env.OBSERVE_MODEL, messages, batch })).digest('hex');
      const validate = payload => {
        if (!payload || !Array.isArray(payload.clusters)) throw new LlmSchemaError('clusters must be an array');
        const converted = clustersToCandidates(payload, batch, { inputPaths: new Set(batch.map(v => v.path)), projects: options.projects, knowledgeSubfolders: options.knowledge_subfolders });
        if (converted.errors.length) {
          const summary = converted.errors.slice(0, 3).map(error => `cluster ${error.cluster_index + 1}: ${error.message}`).join('; ');
          throw new LlmSchemaError(`invalid clusters (${converted.errors.length}): ${summary}`.slice(0, 200));
        }
        return { payload, converted };
      };
      const { payload, converted } = Object.hasOwn(completed, key)
        ? validate(completed[key]) : await request(env, messages, validate);
      // Persist each validated response before the next request can fail. Cache keys
      // include the current evidence, prompt and model; changed sources are re-read.
      if (!converted.errors.length) onBatch(key, payload);
...
async function cluster(args) {
  ...
  try {
    ...
    const previousScan = args.continueScan && existing.scan?.remaining > 0 ? existing.scan : null;
    if (previousScan && (!Number.isInteger(previousScan.processed) || previousScan.processed < 0 || typeof previousScan.cycle_started_at !== 'string')) throw new Error('invalid harvest scan progress');
    const afterCursor = previousScan ? normalizeObservationPath(previousScan.after) : null;
    const collected = scanObservations({ since: args.since, consumed: checkpoint.consumed, locked, limit: args.limit, after: afterCursor, countRemaining: args.continueScan });
    const processed = (previousScan?.processed ?? 0) + collected.items.length;
    const scan = args.continueScan ? {
      after: collected.items.at(-1)?.path ?? afterCursor,
      remaining: collected.remaining,
      eligible_total: processed + collected.remaining,
      processed,
      cycle_started_at: previousScan?.cycle_started_at ?? new Date().toISOString(),
    } : null;
    const batches = buildInputBatches(collected.items);
    const progress = readJson(PROGRESS_PATH, { version: 1, batches: {} });
    if (progress.version !== 1 || !progress.batches || typeof progress.batches !== 'object' || Array.isArray(progress.batches)) throw new Error('invalid harvest progress');
    const env = batches.length ? readEnv() : null;
    const first = batches.length ? await firstLayer(env, batches, options, checkpoint, {
      completed: progress.batches,
      onBatch: (key, payload) => {
        if (!args.dryRun) { progress.batches[key] = payload; atomicJson(PROGRESS_PATH, progress); }
      },
    }) : { candidates: [], errors: [] };
    ...
    const document = { generation: new Date().toISOString(), candidates, batch_errors: batchErrors, options, ...(scan ? { scan } : {}) };
    if (!args.dryRun && (!args.continueScan || !batchErrors.length)) {
      ...
      if (!batchErrors.length) atomicJson(PROGRESS_PATH, { version: 1, batches: {} });
    }
    ...
    process.stdout.write(JSON.stringify({
      ...
      ...(scan ? { scan: { remaining: collected.remaining, eligible_total: scan.eligible_total, processed: scan.processed } } : {}),
    ...
    if (batchErrors.length) process.exitCode = 1;
  } finally { releaseLock(); }
}
...
async function auto() {
  ...
  try {
    ...
    if (summary.errors.length || state.document.candidates.some(candidate => candidate.status === 'needs_check')) process.exitCode = 1;
  } finally { releaseLock(); }
}
