...
import { sundayHealth } from './brain-sunday.mjs';
...
let harvestHealth;
try { harvestHealth = sundayHealth(VAULT_ROOT); }
catch (error) { harvestHealth = { healthy: false, status: 'unreadable', error: error.message }; }
const harvestSummary = `自动提炼：${harvestHealth.healthy ? '正常' : '需处理'}（${harvestHealth.status}）；最后成功：${harvestHealth.last_success || '未确认'}`;
  ...
  : launchRows.length
    ? `<ul class="plain-list">${launchRows
        .map(([label, pid, status]) => {
          ...
          const state = running ? '运行中' : failed ? `上次失败（退出 ${status}）` : status === '0' ? '空闲（上次退出 0）' : '空闲（尚无结果）';
          return `<li class="launch-row"><span class="status-dot ${running ? 'healthy' : failed ? 'unhealthy' : 'idle'}" aria-label="${escapeHtml(state)}"></span><span class="truncate" title="${escapeHtml(label)}">${escapeHtml(label)}</span><span class="pid">${escapeHtml(state)}</span></li>`;
        })
        .join('')}</ul>`
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .status-dot.idle { background:#8a8f98; }
...
<body>
  <main>
    ...
    <footer>${escapeHtml(harvestSummary)}</footer>
  </main>
</body>
</html>
`;
