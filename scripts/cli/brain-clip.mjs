...
// Commands: stop | status | review | approve | reject (stop/status for hand
...
import { callDeepSeek, isValidTimestamp, isValidLayer, assertInsideVault, clipStatePaths, isClipRejected } from '../lib/clip-utils.mjs';
...
function writeClipNote(data, llm, body, attachmentRel, provenance, pendingSha) {
  ...
  const args = [
    ...
    '--clip-id', data.filename,
    '--clip-sha256', pendingSha,
  ];
...
async function approvePendingAsync(ts) {
  ...
  clipStatePaths(ts, VAULT_ROOT);
  if (isClipRejected(ts, VAULT_ROOT)) throw new Error('clip rejected; consumption refused');
  if (!existsSync(jsonPath)) { console.error(`Not found: ${ts}`); process.exit(1); }

  const pendingBytes = readFileSync(jsonPath);
  const d = JSON.parse(pendingBytes);
  d.filename = ts;
  if (!d.llm) {
    console.error(`Cannot approve: no classification data (reason: ${d.reason || 'unknown'}). Retain it or use reject ${ts} --source <actor> --reason <reason>.`);
    process.exit(1);
  }
  const llm = d.llm;
  ...
  let receipt;
  try {
    receipt = writeClipNote(d, llm, body, attachmentRel, provenance, createHash('sha256').update(pendingBytes).digest('hex'));
  } catch (error) {
...
function printHelp() {
  ...
  console.log('  reject <ts> --source <actor> --reason <text>  Recoverably reject through brain-write');
  console.log('');
...
async function main() {
  ...
  switch (command) {
    ...
    case 'reject':
      {
        if (args.length !== 6 || args[2] !== '--source' || args[4] !== '--reason') {
          throw new Error('Usage: reject <ts> --source <actor> --reason <text>');
        }
        const result = spawnSync(process.execPath, [BRAIN_WRITE, '--reject-clip', args[1],
          '--source', args[3], '--reason', args[5]], { encoding: 'utf8', input: '' });
