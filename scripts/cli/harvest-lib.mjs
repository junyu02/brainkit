...
import { structuredJson } from '../lib/memory-records.mjs';
...
export function llmRequestOptions(body, timeoutMs = 120_000) {
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1) throw new Error('LLM timeout must be a positive integer');
  // DeepSeek high reasoning can consume the output limit before emitting JSON.
  const reasoning = /^deepseek-(flash|v4-(flash|pro))$/.test(body.model) ? { reasoning_effort: 'low' } : {};
  return { body: JSON.stringify({ ...body, ...reasoning }), signal: AbortSignal.timeout(timeoutMs) };
}
...
export function parseObservationMarkdown(text, path) {
  ...
  let structured_kind = null;
  for (const language of ['brainkit-candidate', 'brainkit-record', 'brainkit-event']) {
    try {
      if (structuredJson(match[2], language) !== null) structured_kind = language;
    } catch (error) {
      throw new HarvestValidationError(`invalid structured ${language}: ${error.message}`);
    }
  }
  return {
...
export function observationEligible(item, { since, consumed = {}, locked = new Set() } = {}) {
  return item.tags.includes('durable-candidate')
    && !item.structured_kind
    && !item.tags.includes('chronicle') && !/^08-观察\/chronicle-/.test(item.path)
