...
test('2 static checks reject undeclared scripts and scoped conflict copies while missing sources reach repo-removed', () => {
  ...
  // 33 after the memory runtime and HTTP adapter joined the publish surface. The same lock exists in
  // test-plist-render.mjs; both are here so the publish face cannot grow
  // without it showing up in a diff.
  assert.equal(shippedWhitelist.length, 33);
...
test('7 pre-commit revalidation preserves third values and rolls back safe source-parent failures', () => {
  {
...
`);
    ...
    assert.match(result.stderr, /third value/);
...
test('automatic rollback preflights all targets and manifest before restoring or restarting services', () => {
  for (const drift of ['later-target', 'manifest']) {
    const fixture = makeFixture(`rollback-${drift}`, {
      files: { 'scripts/a.txt': 'base-a', 'scripts/b.txt': 'base-b' },
      services: { 'com.second-brain.clip': { loaded: true } },
    });
    changeRepoFile(fixture, 'scripts/a.txt', 'next-a');
    changeRepoFile(fixture, 'scripts/b.txt', 'next-b');
    const target = drift === 'manifest' ? manifestPath(fixture.vault) : join(fixture.vault, '00-系统/scripts/b.txt');
    const hook = createHook(fixture.root, `
import { writeFileSync } from 'node:fs';
if (process.argv[2] === 'before-commit') writeFileSync(process.env.TEST_MUTATE_TARGET, 'external value');
`);
    const result = runPublisher(fixture, [], { BRAIN_PUBLISH_TEST_HOOK: hook, TEST_MUTATE_TARGET: target });
    assert.equal(result.status, 2);
    assert.match(result.stderr, /third value/);
    assert.equal(readFileSync(target, 'utf8'), 'external value');
    assert.equal(readFileSync(join(fixture.vault, '00-系统/scripts/a.txt'), 'utf8'), 'next-a');
    const state = JSON.parse(readFileSync(fixture.launchctl.statePath, 'utf8'));
    assert.ok(state.calls.some(call => call.command === 'bootout'));
    assert.equal(state.calls.some(call => call.command === 'bootstrap'), false);
    const failure = jsonLines(result.stderr).find(item => item.backupDir);
    assert.equal(failure.code, 'rollback-failed');
    assert.ok(existsSync(join(failure.backupDir, 'recovery.json')));
  }
});
