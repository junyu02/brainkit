...
// Direct entry drives failures after preflight; the healthy CLI test below
// independently proves that main reaches the same implementation.
...
test('A5: clean-room core install, doctor, and repeated upgrades stay inside temporary paths', () => {
  const fixture = existingVault('empty');
  assert.equal(installExisting(fixture).code, EXIT.OK);
  writeInstallState(fixture.home, { ...readInstallState(fixture.home), status: 'installed' });
  let doctorOutput = '';
  assert.equal(main(['doctor'], { ...fixture.context, stdout: text => { doctorOutput += text; } }), EXIT.OK, doctorOutput);
  for (let run = 0; run < 2; run++) {
    const upgraded = runUpgradeCli(fixture);
    assert.equal(upgraded.code, EXIT.OK, upgraded.output);
    assert.match(upgraded.output, /Upgrade complete/);
  }
  assert.equal(readInstallState(fixture.home).status, 'installed');
  assert.equal(fixture.launchctl.calls.length, 0, 'core-only must not call the real or mocked launchctl');
});
...
test('slice 6: healthy default CLI upgrades without pulling or starting stopped services', () => {
  const fixture = installedMachine();
  const beforeVault = vaultTree(fixture.vault);
  const publisherCalls = fixture.publisher.calls.length;
  fixture.launchctl.calls.length = 0;
  ...
  const beforeState = readInstallState(fixture.home);
  const { code, output } = runUpgradeCli(fixture, [], { run });

  assert.equal(code, EXIT.OK, output);
  assert.match(output, /Upgrade complete/, output);
  assert.notEqual(readInstallState(fixture.home).last_txn, beforeState.last_txn);
  ...
  // Even a no-change upgrade runs through publication and final verification.
  const during = fixture.publisher.calls.slice(publisherCalls).map(call => call.sub);
  assert.equal(during.filter(sub => sub === '--check').length, 2, String(during));
  assert.equal(during.length, 3, String(during));
});
...
//
// F2/F4 are executable acceptance checks. Reopening the CLI additionally needs
// independent review of the exact commit; passing these alone is insufficient.
for (const seam of ['before-upgrade-commit', 'after-upgrade-commit', 'during-mark', 'after-manifest', 'during-backups', 'after-backups', 'before-rmdir']) {
  test(`F2: committed upgrade cleanup resumes after ${seam}`, () => {
    const fixture = installedMachine();
    const first = runUpgradeApply(fixture, [], {
      failpoint: point => { if (point === seam) throw new Error('interrupted'); },
    });
    assert.equal(first.code, EXIT.RECOVERY, first.stderr);
    const state = readInstallState(fixture.home);
    assert.equal(state.status, 'installed');
    const dir = join(fixture.home, '.config', 'second-brain', 'recovery', state.last_txn);
    const live = join(dir, 'transaction.json');
    if (existsSync(live)) {
      const manifest = JSON.parse(readFileSync(live, 'utf8'));
      for (const operation of manifest.operations) {
        if (operation.backup) assert.ok(existsSync(operation.backup), operation.backup);
      }
    } else {
      assert.equal(JSON.parse(readFileSync(join(dir, 'transaction.spent.json'), 'utf8')).phase, 'committed');
    }
    const installed = vaultTree(fixture.vault);
    let diagnosis = '';
    assert.equal(recoverInstall({ ...fixture.context, stdout: text => { diagnosis += text; } }), EXIT.RECOVERY);
    assert.match(diagnosis, /closeout is unfinished/);
    assert.deepEqual(treeDiff(installed, vaultTree(fixture.vault)), UNCHANGED);
    const retry = runUpgradeCli(fixture, ['--no-pull']);
    assert.equal(retry.code, EXIT.OK, retry.stderr || retry.output);
    assert.match(retry.output, /Previous committed upgrade cleanup completed/);
    assert.equal(existsSync(dir), false, 'the managed retry completes cleanup');
    assert.deepEqual(treeDiff(installed, vaultTree(fixture.vault)), UNCHANGED);
  });
}

test('F2: changed installed bytes retain the cleanup record and backups', () => {
  const fixture = installedMachine();
  assert.equal(runUpgradeApply(fixture, [], {
    failpoint: point => { if (point === 'after-upgrade-commit') throw new Error('interrupted'); },
  }).code, EXIT.RECOVERY);
  const dir = join(fixture.home, '.config', 'second-brain', 'recovery', readInstallState(fixture.home).last_txn);
  const before = vaultTree(dir);
  const shim = join(fixture.home, '.local', 'bin', 'brain-node');
  writeFileSync(shim, 'third party bytes');
  const retry = runUpgradeCli(fixture, ['--no-pull']);
  assert.equal(retry.code, EXIT.RECOVERY, retry.stderr);
  assert.match(retry.stderr, /artifact changed/);
  assert.equal(readFileSync(shim, 'utf8'), 'third party bytes');
  assert.deepEqual(treeDiff(before, vaultTree(dir)), UNCHANGED);
});
...
test('F4: step 9 rejects a failed publisher even with a clean summary', () => {
...
for (const changed of ['plist-bytes', 'loaded-plist']) {
  test(`F4: step 9 rejects changed ${changed}`, () => {
    const fixture = installedMachine({ components: 'core,watch' });
    let checks = 0;
    let verifying = false;
    const path = readInstallState(fixture.home).plists.watch.path;
    const run = (command, args = [], options) => {
      if (args.includes('--check') && ++checks > 1) {
        verifying = true;
        if (changed === 'plist-bytes') {
          const text = readFileSync(path, 'utf8');
          assert.ok(text.includes('brain-watch-wrapper.sh'));
          writeFileSync(path, text.replace('brain-watch-wrapper.sh', 'different-wrapper.sh'));
        }
      }
      if (changed === 'loaded-plist' && verifying && command === fixture.context.launchctlPath && args[0] === 'print') {
        return { status: 0, stdout: `path = ${join(fixture.home, 'foreign.plist')}\n`, stderr: '' };
      }
      return fixture.context.run(command, args, options);
    };
    const { code, output } = runUpgradeApply(fixture, [], { run });
    assert.notEqual(code, EXIT.OK, output);
    assert.match(output, changed === 'plist-bytes' ? /FAIL  plist-watch/ : /FAIL  service-com.second-brain.watch/);
  });
}
