...
test('archive refuses an external destination before any candidate moves', () => {
  const fixture = makeFixture();
  const note = writeNote(fixture.vault, '03-经验/shared/note.md', { name: '保留', body: 'keep source' });
  const outside = join(fixture.root, 'outside');
  mkdirSync(outside);
  mkdirSync(join(fixture.vault, '06-归档'));
  symlinkSync(outside, join(fixture.vault, '06-归档/03-经验'));
  const before = readFileSync(note);
  const result = runArchive(fixture, ['--apply', '--json']);
  assert.notEqual(result.status, 0, result.stdout);
  assert.deepEqual(readFileSync(note), before);
  assert.deepEqual(readdirSync(outside), []);
});

test('archive refuses a linked memory index and retains the note and external bytes', () => {
  const fixture = makeFixture();
  const note = writeNote(fixture.vault, '03-经验/note.md', { name: '保留', body: 'keep source' });
  const outside = join(fixture.root, 'outside.md');
  writeFileSync(outside, '# external sentinel\n');
  symlinkSync(outside, join(fixture.memory, 'MEMORY-archive.md'));
  const result = runArchive(fixture, ['--apply', '--json']);
