...
test('brain-status generates reports with env roots and unavailable mempalace', () => {
  ...
  writeFileSync(join(ROOT, 'empty-path', 'launchctl'), '#!/bin/sh\nprintf "%s\\n" "PID Status Label" "- 0 com.second-brain.idle" "- 1 com.second-brain.failed" "123 1 com.second-brain.running" "- - com.second-brain.new"\n', { mode: 0o755 });
  const withTasks = spawnSync(process.execPath, [STATUS], {
    encoding: 'utf8',
    env: { ...process.env, PATH: join(ROOT, 'empty-path'), BRAIN_VAULT_ROOT: VAULT,
      BRAIN_MEMORY_DIR: MEMORY, BRAIN_CLAUDE_SESSIONS_ROOT: join(ROOT, 'claude'),
      BRAIN_CODEX_SESSIONS_ROOT: join(ROOT, 'codex'), BRAIN_CHRONICLE_ROOT: join(ROOT, 'chronicle'),
      MEMPALACE_BIN: join(ROOT, 'missing-mempalace') },
  });
  assert.equal(withTasks.status, 0, withTasks.stderr);
  const html = readFileSync(join(VAULT, '状态总览.html'), 'utf8');
  assert.match(html, /class="status-dot idle" aria-label="空闲（上次退出 0）"/);
  assert.match(html, /class="status-dot unhealthy" aria-label="上次失败（退出 1）"/);
  assert.match(html, /class="status-dot healthy" aria-label="运行中"/);
  assert.match(html, /class="status-dot idle" aria-label="空闲（尚无结果）"/);
});
