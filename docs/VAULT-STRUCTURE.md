...
| 来源 | 目标 | 触发方式 |
|------|------|----------|
| 本机已注册项目的文件变化 | Vault `01-项目/<项目>/_index.md` | `brain-watch` 更新项目卡片；新项目由 `brain-init` 注册，SessionStart 不写 `MEMORY.md` |
| AI 会话 | `08-观察/` | `observe` 定时生成摘要；`04-对话/` 仅保存精选内容 |
| 剪贴板 | `raw/pending/`、`08-观察/` | 受管采集与 `observe --clips` 摄取；批准、拒收走受管入口 |
| 合格观察 | 长期知识、经验或项目条目 | `harvest` 提炼、复核和去重后经 writer 写入 |
| Vault 文件 | MemPalace 只读索引 | 受管 watcher 单向同步，查询端筛选活动内容 |

## 归档策略

| 资产 | 保留策略 |
|------|----------|
| `.trash/` | 历史停用材料永久保留，不安排自动永久删除 |
| `raw/` | 原始输入与恢复材料永久保留；拒收、停用和隔离只改变活动状态，不销毁原文 |
