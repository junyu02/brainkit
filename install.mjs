...
//
// `committed` is the upgrade's point of no return, and carries two invariants
// that are enforced, not assumed. settleTransaction refuses to roll back a
// committed transaction at all (an edited-in `committed` therefore cannot cause
// an unwind, only a refusal). And finishUpgradeCleanup re-reads the disk before
// clearing one: every operation must still match its post-image, so a manifest
// that claims `committed` while the artifacts say otherwise is refused too.
const TXN_PHASES = ['active', 'rolled-back', 'committed'];
...
function loadTransaction(home, id, claimedVaultRoot, claimedComponents, requiredness) {
  ...
  if (spent && !['rolled-back', 'committed'].includes(txn.phase)) {
    throw new InstallError(`${join(dir, TXN_SPENT)} is marked spent but does not record a finished rollback or committed upgrade`, EXIT.UNSAFE);
  }
  txn.spent = spent;
...
function settleTransaction(txn, verify = () => [], expectations = {}) {
  if (txn.phase === 'committed') {
    return { tag: 'blocked', problems: ['upgrade is committed; only forward cleanup is allowed'], done: [] };
  }
...
function closeOut(txn, done, { releaseState = true } = {}) {
  if (!releaseState && txn.phase !== 'committed') return { tag: 'blocked', problems: ['success must be durably committed before cleanup'], done };
  ...
  if (!txn.spent && releaseState) {
    const failed = removeBackups();
...
function recoverInstall(context) {
  ...
  if (state.status === 'installed') {
    ...
    if (txn && isUpgradeCloseout(txn)) {
      const problems = upgradeCloseoutProblems(txn, state);
      context.stdout([
        'Upgrade files are installed; transaction closeout is unfinished.',
        `Recovery material: ${txn.dir}`,
        ...problems,
        problems.length ? 'Files changed after installation; recovery material is retained.'
          : 'Run node install.mjs upgrade --no-pull to finish forward cleanup. Do not roll this transaction back.',
        '',
      ].join('\n'));
...
// Does this transaction look like an upgrade that got as far as rewriting
// install-state? Either it is already `committed`, or it is still `active` and
// the state file has both a before- and an after-image -- which only an upgrade
// produces, since a fresh install has no state to take a pre-image of.
//
// ponytail: shape inference, not a declared intent. It holds today because
// install is the only other writer of this anchor and it never has a `pre`.
// If an install transaction ever reaches status=installed with one, this reads
// it as an upgrade closeout. The fix then is an explicit intent field on the
// manifest rather than a fourth clause here.
function isUpgradeCloseout(txn) {
  const anchor = txn.operations.find(operation => operation.path === installStatePath(txn.home));
  return txn.phase === 'committed' || (txn.phase === 'active' && Boolean(anchor?.pre && anchor?.post));
}

// `state.last_txn === txn.id` used to be checked here too. It cannot fail: both
// callers get their txn from loadTransaction(home, state.last_txn, ...), so the
// id came out of the very field it was compared against. A check that cannot
// fail reads as protection and provides none.
function upgradeCloseoutProblems(txn, state) {
  if (state.status !== 'installed') return ['install state does not identify a completed upgrade'];
  const stateOperation = txn.operations.find(operation => operation.path === installStatePath(txn.home));
  if (!stateOperation?.pre || !stateOperation.post) return ['upgrade lacks an existing install-state before-image'];
  return txn.operations.filter(operation => operationState(operation, txn) !== (operation.post ? 'installed' : 'undone'))
    .map(operation => `completed upgrade artifact changed: ${operation.path}`);
}

function finishUpgradeCleanup(context, state) {
  const txn = loadTransaction(context.home, state.last_txn, state.vault_root, state.components, state.plan_digest);
  if (!txn || !isUpgradeCloseout(txn)) return;
  const problems = upgradeCloseoutProblems(txn, state);
  if (problems.length) throw new InstallError(problems.join('; '), EXIT.RECOVERY);
  if (txn.phase !== 'committed') { txn.phase = 'committed'; commitTransaction(txn); }
  const result = closeOut(txn, [], { releaseState: false });
  if (!['settled', 'swept'].includes(result.tag)) {
    throw new InstallError(`forward cleanup remains: ${result.problems.join('; ')}`, EXIT.RECOVERY);
  }
  context.stdout('Previous committed upgrade cleanup completed.\n');
  return true;
}
...
function runUpgrade(context, options) {
  ...
  try {
    if (finishUpgradeCleanup(context, state)) return EXIT.OK;
    ...
    // F2/F4 acceptance and independent S3 review passed at a6f28eb.
    return upgradeApply(context, answers, state);
  } finally {
    release();
  }
}
...
//
// Step 9 has landed: doctor runs here, and its verdict is authority -- a
// failure past the commit point is reported, not rolled back. That asymmetry is
// deliberate. Up to the commit, failure unwinds through the same
// settleTransaction applyPlan uses. Once the manifest reaches `committed` the
// upgrade IS the machine's state, and undoing it would throw away a working
// install to satisfy a check. Only the component smoke tests are still deferred
// to the real-machine batch, which the success summary says outright.
function upgradeApply(context, answers, state) {
  ...
  try {
    ...
    upgradeRecorded = true;
    txn.failpoint?.('before-upgrade-commit');
    txn.phase = 'committed';
    commitTransaction(txn);
    txn.failpoint?.('after-upgrade-commit');
    ...
    if (closed.tag !== 'settled' && closed.tag !== 'swept') {
      throw new InstallError(
    ...
    if (upgradeRecorded) {
      process.stderr.write(`Upgrade files are installed; forward cleanup remains at ${txn.dir}: ${error.message}\n`
        + 'Run node install.mjs upgrade --no-pull to resume cleanup; installed files were not rolled back.\n');
      return EXIT.RECOVERY;
    }
...
function verifyUpgrade(context, { answers, vaultRoot, plists, wasLoaded, shims }) {
  ...
  try {
    assertPublisherStatus(after, [0], 'publish --check');
    const summaries = publisherRecords(after).filter(entry => entry.type === 'summary');
    if (summaries.length !== 1 || summaries[0].state !== 'clean' || summaries[0].exitCode !== 0) {
      throw new InstallError('publish --check did not produce exactly one clean, exitCode=0 summary', EXIT.UNSAFE);
    }
  } catch (error) { fail('published-code', error.message); }
  ...
  for (const [service, written] of Object.entries(plists)) {
    ...
    try {
      ...
      const actual = readManagedFile(path, `${service} plist`);
      if (!/^[a-f0-9]{64}$/.test(written.sha256 || '') || sha256(actual.content) !== written.sha256) {
        throw new InstallError(`${service} plist differs from the bytes this upgrade wrote`, EXIT.UNSAFE);
      }
    } catch (error) {
  ...
  for (const label of wasLoaded) {
    try {
      ...
      const expected = plists[label.replace('com.second-brain.', '')]?.path;
      if (!found.loaded || !samePlist(found.plist, expected)) {
        fail(`service-${label}`, `${label} is not loaded from this upgrade's plist ${expected}`);
      }
    } catch (error) { fail(`service-${label}`, error.message); }
  }
  return failures;
}
...
export {
  ...
  uninstallPlan,
  // Direct entry for failure injection after preflight; main reaches it too.
  upgradeApply,
